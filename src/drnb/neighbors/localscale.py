...
def example_usage():
    """
    This function demonstrates how to use locally_scaled_neighbors with
    synthetic data. It is not part of the main API and can be removed or
    modified for your actual use case.
    """
    # Suppose we have an N=10 dataset
    N = 10
    k = 8  # We artificially pick k=8 neighbors (including self as col0)
    np.random.seed(42)

    # Indices: the first column is the self-neighbor (i). Then we fill with
    # random neighbors just for demonstration.
    idx = np.zeros((N, k), dtype=np.int64)
    for i in range(N):
        idx[i, 0] = i  # self
        choices = np.arange(N)
        np.random.shuffle(choices)
        idx[i, 1:] = choices[choices != i][: k - 1]

    # Distances: first column is zero for the self neighbor
    dist = np.random.rand(N, k)
    dist[:, 0] = 0.0

    # Let's request l=5 local neighbors from the top-m=8 extended neighbors.
    # We'll compute sigma from neighbors [scale_from=5 .. scale_to=7].
    # (Remember these are 1-based positions.)
    l = 5
    m = 8
    final_idx, final_dist = locally_scaled_neighbors(
        idx, dist, l=l, m=m, scale_from=5, scale_to=7
    )

    print("Original Indices:\n", idx)
    print("Original Distances:\n", dist)
    print("\nLocally Scaled (l=5, m=8) Indices:\n", final_idx)
    print("Locally Scaled (l=5, m=8) Distances:\n", final_dist)
